...
The Model object contains high-level **components** (Python objects) that hold the data about specific elements in the power system, for example power plants, demand, transmission etc. [High-level components can be represented as low-level components](components.md)  - flows and nodes. For example, demand is a flow with an arrow towards a node, while production is a flow with an arrow away from a node.
